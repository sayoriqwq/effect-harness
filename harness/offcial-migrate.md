...
第二阶段不照抄 `mikearnaldi/accountability` 的业务、前端架构或旧 Effect 写法，只保留工程化理念：
项目约束应写成稳定文档、provider contract 和可执行 verifier。
...
- agent guide 和 route table 要稳定。
- 项目约束要可执行。
- 完成工作前必须跑统一 verifier。
- provider 与 target lifecycle 要分层。
- diagnostics 覆盖的语义不重复写自定义 lint。
...
`pnpm source:verify` 借用 Partita pin verifier。

`pnpm effect:verify` 验证 source entries、provider profile、strict tsgo policy、package baseline、
rule map、diagnostic suppressions 和 import boundary。

`pnpm verify` 继续运行 `tsgo --noEmit`、tests、eslint 和 knip。
