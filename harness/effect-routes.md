# Effect Source Routes

本文件是 agent 读取 `repos/effect/` 的路线表。它不是通用 pin contract；通用外部真源接入由
Partita 负责。本仓只维护已经 pin 进来的 Effect source-entry 实例，并把它整理成可读取、可核对、
可验证的参考入口。

## 读取边界

- `repos/effect/` 是 provider 仓内部只读参考源，target 仓库不接收这棵目录。
- 业务代码和测试代码只能从已安装 package import，禁止从 `repos/effect` import。
- `repos/effect/LLMS.md` 是上游 LLM guide，总是优先于零散猜测和 `node_modules`。
- 是否追上最新官方上游，以 `pnpm effect:status` 为准；不要从本文档推断 latest。
- 更新 source pin 只能走 `pnpm effect:update`，然后运行 `pnpm verify`。

## Route 表

| Agent 意图 | 先读 | 深入核对 | 适用输出 | 注意 |
| --- | --- | --- | --- | --- |
| 判断当前 source pin 和 package baseline | `repos/effect.subtree.json`、`pnpm effect:status` | `harness/provider/effect-harness.provider.json`、`pnpm-workspace.yaml` | baseline 审计、provider drift 判断 | `effect:status` 可能显示当前 pin 落后官方；落后不等于本仓 baseline 不自洽 |
| 理解本仓如何接入 Effect source | `harness/source.md` | `harness/provider/index.md`、`harness/offcial-guide.md` | source-entry 维护、更新计划 | 这里是 Effect 实例，不是 Partita generic pin |
...
| 写 Node 入口 | `repos/effect/ai-docs/src/01_effect/05_running/10_run-main.ts` | `repos/effect/packages/platform-node/src/NodeRuntime.ts`、`NodeServices.ts` | CLI/server entrypoint | Node target 使用 `NodeRuntime.runMain` |
...
| 查模块行为细节 | 对应 `repos/effect/packages/effect/test/<Module>.test.ts` | `repos/effect/packages/effect/typetest/` | 行为断言、类型边界 | 测试是行为参考，不是 target 代码的 import 来源 |
...
接入 effect-harness 的 target 项目应得到 Effect v4 beta baseline、`tsgo --noEmit` 诊断路径、
`@effect/language-service` floatingEffect 约束、以及 provider record 中的 source identity。target
不应该得到 `repos/effect/` 本体、repo-local Codex skills、runtime templates、feedback intake、
`.effect-harness.json` 或 effect-harness 管理的 `AGENTS.md` block。
