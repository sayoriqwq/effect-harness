...
这是 provider 仓内部的 source-entry。目标项目不接收也不维护 `repos/effect`、
...
- `repos/effect.subtree.json` 记录 repository、branch、prefix、split、LLM 文档路径和 package 基线。
- `.partita/source-entries.json` 记录通用 source-entry pin contract，由 Partita source CLI
...
- `pnpm effect:verify` 检查 source 是否存在、是否误用 gitlink/submodule、LLM doc 是否存在、
  package 基线是否一致，以及应用代码是否 import 了 pinned source。
  Git history 必须包含与 manifest split 对齐的 subtree trailer；只改 manifest 的源入口 pin 不通过验证。
...
- provider profile 暴露 source-entry identity 和 package 基线。
...
只有准备好评审源入口 pin 与 package 基线更新时，才从干净工作区走 Partita source 流程和显式
git subtree 更新。更新后：
