...
源入口实例、读取路线、Effect 基线、strict tsgo policy、provider profile 和本地 fixture。
...
| Provider 层 | 让下一阶段 Prelude 知道如何集成 effect-harness | 通过 provider profile 声明目标项目应接收的基线、诊断入口、source identities 与 docs bundle | 直接写目标 runtime、`.codex`、`.effect-harness.json`、目标 `AGENTS.md` 管理块 |
...
- `harness/provider/effect-harness.provider.json`：provider profile，也是 package 基线真源。
- `fixture/`：当前阶段的 gitignored 本地标准输出项目，必须可以独立运行。

## 职责分配

- Partita 负责通用源入口 pin 流程。
- effect-harness 负责 Effect/tsgo 具体源入口实例和 provider profile。
- 当前阶段先维护 `fixture/` 的独立运行能力。
- Prelude 集成推到下一阶段，再消费 provider profile 并维护目标项目的 provider record、drift、
  verify 和 maintain。
...
目标项目由下一阶段 Prelude provider maintain/verify；当前阶段 effect-harness 不直接向目标项目投影
runtime 资产。
