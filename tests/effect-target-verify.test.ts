...
it.effect('target verifier rejects caret ranges for pinned Effect baseline dependencies', () => Effect.sync(() => {
  ...
  try {
    ...
    packageJson.dependencies.effect = '^4.0.0-beta.78'
    ...
    assert.match(result.stderr, /effect is \^4\.0\.0-beta\.78; expected 4\.0\.0-beta\.78 or catalog:/u)
  }
  finally {
...
it.effect('target verifier rejects local effect harness dispatcher scripts', () => Effect.sync(() => {
  ...
  try {
    ...
    const packageJsonPath = join(target, 'package.json')
    const packageJson = JSON.parse(readFileSync(packageJsonPath, 'utf8'))
    packageJson.scripts['effect:verify'] = 'node scripts/effect-harness.mjs verify'
    writeFileSync(packageJsonPath, `${JSON.stringify(packageJson, null, 2)}\n`)
    mkdirSync(join(target, 'scripts'))
    writeFileSync(join(target, 'scripts/effect-harness.mjs'), '#!/usr/bin/env node\n')
