...
it.effect('effect-harness init installs consumer runtime and target contract', () => Effect.sync(() => {
  ...
  try {
    ...
    assert.equal(packageJson.dependencies.effect, '4.0.0-beta.90')
    assert.equal(packageJson.dependencies['@effect/platform-node'], '4.0.0-beta.90')
    assert.equal(packageJson.devDependencies['@effect/vitest'], '4.0.0-beta.90')
    ...
    assert.equal(packageJson.devDependencies['@typescript/native-preview'], '7.0.0-dev.20260624.1')
...
it.effect('effect-harness init preserves target catalog dependencies and updates catalog versions', () => Effect.sync(() => {
  ...
  try {
    ...
    assert.match(workspace, /effect: 4\.0\.0-beta\.90/u)
    assert.match(workspace, /'@effect\/platform-node': 4\.0\.0-beta\.90/u)
    assert.match(workspace, /'@effect\/vitest': 4\.0\.0-beta\.90/u)
    assert.match(workspace, /'@effect\/tsgo': 0\.14\.6/u)
    assert.match(workspace, /'@effect\/language-service': 0\.86\.2/u)
    assert.match(workspace, /'@typescript\/native-preview': 7\.0\.0-dev\.20260624\.1/u)
