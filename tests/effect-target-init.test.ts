...
it.effect('effect-harness init installs consumer runtime and target contract', () => Effect.sync(() => {
  ...
  try {
    ...
    assert.equal(packageJson.dependencies.effect, '4.0.0-beta.78')
    assert.equal(packageJson.dependencies['@effect/platform-node'], '4.0.0-beta.78')
    assert.equal(packageJson.devDependencies['@effect/vitest'], '4.0.0-beta.78')
    assert.equal(packageJson.devDependencies['@effect/tsgo'], '0.14.0')
    assert.equal(packageJson.devDependencies['@effect/language-service'], '0.86.2')
    assert.equal(packageJson.devDependencies['@typescript/native-preview'], '7.0.0-dev.20260606.1')
