...
it.effect('checks source-pin freshness through an isolated read-only publication route', () => Effect.sync(() => {
  ...
  expect(verifier).toMatch(/'pin',\s*'publish'/u)
  ...
  expect(verifier).toMatch(/'diff',\s*'--exit-code',\s*'HEAD'/u)
...
it.effect('keeps packed PREPARE separate and accepts either converged or expected-drift Partita preflight', () => Effect.sync(() => {
  ...
  expect(acceptance.indexOf('const decodePreparedTargetEvidence')).toBeLessThan(acceptance.indexOf('\ntry {'))
