...
`effect-harness init` 产物、target verifier、`harness/runtime/codex/` 和 AGENTS route fragment
要彼此一致。
...
1. 读 harness contract，而不是本地教程：
   `README.md`、`harness/index.md`、
   `harness/official-inventory.md`、`harness/exposure.md`、
   `harness/target-agent-contract.md`、`harness/feedback/index.md`、`repos/effect.subtree.json`。
2. 修改目标仓库接入面时，同时检查：
...
- 不把本仓库 `.codex/skills/` 投递给 target repo；target 只接收 `harness/runtime/codex/`。
